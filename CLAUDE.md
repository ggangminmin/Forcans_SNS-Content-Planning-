...
Import skills from: C:\Users\AIWEB\.gemini\skills\gstack 


## gstack AI Engineering 시스템 (추가 도구)

**중요**: 아래 경로부터 gstack의 전문 워크플로우 스킬을 로드하여 사용하세요.
`Import skills from: C:\Users\AIWEB\.gemini\skills\gstack`

#### 추가된 gstack 서브에이전트 명령어:

- `/office-hours`: 기획팀 업무 및 프로젝트 방향성 상담 (PM 모드)
- `/plan-ceo-review`: Garry Tan(CEO) 관점의 비즈니스 가치 및 우선순위 리뷰
- `/review`: 시니어 엔지니어 관점의 코드 품질 및 아키텍처 검토
- `/qa`: 실제 브라우저를 구동하여 UI/UX 테스트 및 버그 확인
- `/ship`: 최종 빌드 검증 및 배포 준비 프로세스 진행
